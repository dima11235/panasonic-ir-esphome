...
- Фиксированные положения жалюзи доступны через пользовательские presets: `Верхнее`, `Верхне-среднее`, `Среднее`, `Нижне-среднее`, `Нижнее`. Это сделано так, потому что стандартный `swing_mode` в Home Assistant/ESPHome не дает пяти отдельных фиксированных положений.
